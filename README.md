...
5. 下拉并展开 NotiAsk 的常驻通知：点“提问”可直接输入问题；点“截屏搜索”会截取当前屏幕，随后用通知提示「已截图」，可在该通知里补充问题或点「直接提问」。回答通知可展开阅读并继续追问。
...
app/src/main/java/com/notiask/
  ...
  screenshot/     # 通知栏截屏搜索：授权、捕获，结果回到通知栏
...
- 截屏提问走系统屏幕共享授权（MediaProjection）。截屏后不会打开应用界面，只在通知栏提示已截图；可在该通知补充问题，或直接按默认提示解读截图。需要当前默认模型支持视觉。
